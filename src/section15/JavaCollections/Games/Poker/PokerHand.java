...
public class PokerHand {
    ...
    public void evalHand() {
        ...
        //  making a list with all the faces of the cards;
        List<String> faceList = new ArrayList<>(hand.size());
        hand.forEach(card -> faceList.add(card.face()));

        //  making a list with only duplicate faces;
        //  why?
        //  so we can iterate through unique values on the next step;
        ...
        //  here we iterate through unique values;
        //  and setRank() based on their frequency;
        for (String duplicate : duplicateFaceCards) {
            //  we don't need to worry about order on faceList;
            //  because hands, from where it is derived, has already been ordered on the constructor!
            int start = faceList.indexOf(duplicate);
            int last = faceList.lastIndexOf(duplicate);
            setRank(last - start + 1);

            //  passing the duplicate cards to the keepers list;
