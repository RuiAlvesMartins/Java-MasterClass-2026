...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        lidl.getAisleInventory().keySet().forEach(k -> {
            System.out.println(" %s %s %s ".formatted("-".repeat(20), k, "-".repeat(20)));
            lidl.getAisleInventory().get(k).keySet().forEach(
                k2 -> System.out.println(lidl.getAisleInventory().get(k).get(k2)));
        });

        var storeAisle = lidl.getAisleInventory();

        Cart c1 = new Cart(StoreType.PHYSICAL);
        lidl.getCarts().add(c1);

        c1.addItem(storeAisle.get(Category.VEGETABLES).get("Onions"), 20);
        c1.addItem(storeAisle.get(Category.VEGETABLES).get("Carrots"), 20);
        c1.addItem(storeAisle.get(Category.DAIRY).get("Eggs"), 10);
        lidl.listProductsByCategory(Category.VEGETABLES, Category.DAIRY);
        c1.printSalesSlip(lidl.getInventory());

        c1.removeItem(storeAisle.get(Category.DAIRY).get("Milk"));
        c1.removeItem(storeAisle.get(Category.DAIRY).get("Eggs"));
        c1.removeItem(storeAisle.get(Category.VEGETABLES).get("Onions"));
        lidl.listProductsByCategory(Category.VEGETABLES, Category.DAIRY);
        c1.printSalesSlip(lidl.getInventory());

        c1.addItem(storeAisle.get(Category.DAIRY).get("Milk"), 60);
        c1.addItem(storeAisle.get(Category.DAIRY).get("Eggs"), 30);
        lidl.listProductsByCategory(Category.VEGETABLES, Category.DAIRY);
        c1.printSalesSlip(lidl.getInventory());
        ...
        c2.addItem(storeAisle.get(Category.DAIRY).get("Milk"), 40);
        c2.addItem(storeAisle.get(Category.DAIRY).get("Eggs"), 10);
        c2.addItem(storeAisle.get(Category.VEGETABLES).get("Onions"), 60);
        ...
        //  for PHYSICAL carts, client picks products from the aisles, i.e. aisleInventory;
        Cart c3 = new Cart(StoreType.PHYSICAL, -1);
        lidl.getCarts().add(c3);
        c3.addItem(storeAisle.get(Category.DAIRY).get("Eggs"), 10);
        //  for ONLINE carts, client choice is mapped to inventory directly;
        Cart c4 = new Cart(StoreType.ONLINE, 1);
        lidl.getCarts().add(c4);
        int iCarrotSKU = storeAisle.get(Category.VEGETABLES).get("Carrots").getProduct().getSku();
        c4.addItem(lidl.getInventory().get(iCarrotSKU), 40);
